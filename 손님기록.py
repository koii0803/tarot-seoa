...
이어세는시간 = 12      # 같은 손님이 다른 글에 달아도 이 시간 안이면 이어서 센다 (2026-09-26)
...
def 안내했나(손님, 글아이디=""):
    키 = _손님키(손님)
    return any(x.get("그림") == "뒷면" for x in _읽기() if _같은자리(x, 키, 글아이디))
...
def _같은자리(줄, 키, 글아이디):
    ...
    **글이 다르면 다른 대화다** (2026-09-24). 예전 기록엔 글아이디가 없어서
    ...
    if 적힌글 == 글아이디:
        return True
    # 2026-09-26 사장님: 같은 손님이 다른 글 밑에 달아도 몇 시간 안이면 같은 대화다
    # (jjjin_a_ 가 4번째 말을 다른 글에 달아서 1턴으로 되돌아갔다)
    try:
        때 = dt.datetime.fromisoformat(줄.get("때") or "")
        if 때.tzinfo is None:
            때 = 때.replace(tzinfo=dt.timezone.utc)
        return (지금() - 때) <= dt.timedelta(hours=이어세는시간)
    except Exception:
        return False
