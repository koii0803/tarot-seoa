...
보류 = 대기표 '실패' (답을 썼는데 검사를 못 넘은 댓글) · '못나감' (스레드가 3번 거절한 답글)
...
def 걸린것들(줄들):
    ...
    # 못나감 = 답은 썼는데 스레드가 3번 거절해서 멈춰 둔 것 (2026-09-25). 사람이 봐야 하니 보류에 넣는다
    보류 = [x for x in 줄들 if x.get("상태") in ("실패", "못나감") and x.get("댓글아이디") not in 답한것]
    return 무시, 보류
